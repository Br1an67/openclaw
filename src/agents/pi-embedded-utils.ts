...
export function parseMinimaxToolCallXml(text: string): ParsedMinimaxToolCall[] {
  ...
  // Only parse <invoke> blocks that belong to minimax tool_call segments
  // (delimited by </minimax:tool_call>). When a segment also contains an
  // opening <minimax:tool_call>, only content after it is considered; this
  // prevents explanatory/code-sample XML from being promoted into toolUse blocks.
  const closingTagRe = /<\/minimax:tool_call>/gi;
  const openingTagRe = /<minimax:tool_call>/gi;
  const sections: string[] = [];
  let lastEnd = 0;
  for (const closeMatch of text.matchAll(closingTagRe)) {
    const closeIdx = closeMatch.index!;
    let segment = text.slice(lastEnd, closeIdx);
    let lastOpenEnd = -1;
    for (const openMatch of segment.matchAll(openingTagRe)) {
      lastOpenEnd = openMatch.index! + openMatch[0].length;
    }
    if (lastOpenEnd >= 0) {
      segment = segment.slice(lastOpenEnd);
    }
    sections.push(segment);
    lastEnd = closeIdx + closeMatch[0].length;
  }
  ...
  for (const match of sections.join("\n").matchAll(invokeRe)) {
    const name = match[1];
    const body = match[2];
