...
describe("isTransientSqliteError", () => {
  ...
  it("returns true for node:sqlite ERR_SQLITE_ERROR with transient message", () => {
    const messages = [
      "database is locked",
      "database table is locked",
      "unable to open database file",
      "SQLITE: database is locked (via node:sqlite)",
    ];
    for (const msg of messages) {
      const error = Object.assign(new Error(msg), { code: "ERR_SQLITE_ERROR" });
      expect(isTransientSqliteError(error), `message: ${msg}`).toBe(true);
    }
  });

  it("returns false for node:sqlite ERR_SQLITE_ERROR with non-transient message", () => {
    const messages = ["disk I/O error", "database disk image is malformed", "out of memory"];
    for (const msg of messages) {
      const error = Object.assign(new Error(msg), { code: "ERR_SQLITE_ERROR" });
      expect(isTransientSqliteError(error), `message: ${msg}`).toBe(false);
    }
  });

  it("returns true for node:sqlite ERR_SQLITE_ERROR nested in cause chain", () => {
    const inner = Object.assign(new Error("database is locked"), { code: "ERR_SQLITE_ERROR" });
